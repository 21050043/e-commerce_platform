export function getStatusColor(status: string) {
  switch (status) {
    ...
    case 'Chờ vận chuyển': return 'bg-sky-100 text-sky-800';
    case 'Đã nhận hàng': return 'bg-amber-100 text-amber-800';
    case 'Đang giao hàng': return 'bg-blue-100 text-blue-800';
    case 'Chờ xác nhận': return 'bg-violet-100 text-violet-800';
